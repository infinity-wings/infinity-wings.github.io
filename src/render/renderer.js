...
const storyWreckFallbackAssets={laser:shipSpriteAssets.players.laser};
function preloadStoryWreckAsset(fighterId='laser'){
 const image=storyWreckAssets[fighterId]||storyWreckAssets.laser;
 if(!image)return Promise.resolve(false);
 if(image.complete&&image.naturalWidth)return typeof image.decode==='function'?image.decode().catch(()=>false).then(()=>true):Promise.resolve(true);
 return new Promise(resolve=>{const done=()=>resolve(Boolean(image.naturalWidth));image.addEventListener('load',done,{once:true});image.addEventListener('error',done,{once:true})});
}
for(const fighterId of Object.keys(storyWreckAssets))preloadStoryWreckAsset(fighterId);
...
function draw(){
 ...
 // 剧情遗骸使用独立前景通道：高于全部战斗模型，低于随后绘制的事件提示与 HUD。
 withRenderState('第三阶段战机遗骸前景',()=>drawWreckStory());
...
function drawWreckStory(){
 ...
 // `finite` belongs to the main draw loop's local scope. Keep this foreground
 // story renderer self-contained so a cinematic can never disappear because
 // that helper is unavailable here.
 const safeNumber=(value,fallback)=>Number.isFinite(value)?value:fallback;
 const requestedAsset=storyWreckAssets[story.fighterId]||storyWreckAssets.laser,fallbackAsset=storyWreckFallbackAssets[story.fighterId]||storyWreckFallbackAssets.laser,asset=requestedAsset?.complete&&requestedAsset.naturalWidth?requestedAsset:fallbackAsset,x=safeNumber(story.x,W/2),y=safeNumber(story.y,150),pulse=.5+.5*Math.sin(performance.now()*.0042);
 ...
 ctx.save();ctx.translate(x,y+safeNumber(story.float,0));ctx.rotate(safeNumber(story.rotation,0));ctx.globalAlpha=Math.max(0,Math.min(1,safeNumber(story.alpha,1)));ctx.shadowBlur=['return','scan','reveal','briefing'].includes(story.phase)?24:19;ctx.shadowColor='#8ca5b5';ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';if(asset?.complete&&asset.naturalWidth){const size=story.phase==='signal'?154:story.phase==='inspect'?218:176,reveal=Math.max(0,Math.min(1,safeNumber(story.revealProgress,0)));ctx.filter='grayscale(1) saturate(0) brightness(.62) contrast(1.28)';ctx.drawImage(asset,-size/2,-size/2,size,size);if(reveal>0){ctx.save();ctx.beginPath();ctx.rect(-size/2,-size/2,size,size*reveal);ctx.clip();ctx.filter='none';ctx.shadowColor='#5cecff';ctx.drawImage(asset,-size/2,-size/2,size,size);ctx.restore()}}else{ctx.fillStyle='rgba(87,96,102,.82)';ctx.strokeStyle='#aebbc2';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(0,-64);ctx.lineTo(64,36);ctx.lineTo(16,26);ctx.lineTo(0,58);ctx.lineTo(-16,26);ctx.lineTo(-64,36);ctx.closePath();ctx.fill();ctx.stroke()}ctx.restore();
 ctx.save();ctx.textAlign='center';if(story.phase==='signal'){ctx.fillStyle='rgba(3,12,27,.88)';ctx.fillRect(W/2-128,H*.29,256,52);ctx.strokeStyle='rgba(85,224,255,.52)';ctx.strokeRect(W/2-128,H*.29,256,52);ctx.fillStyle='#75e8ff';ctx.font='800 12px system-ui';ctx.fillText('系统检测到失联战机信号',W/2,H*.29+22);ctx.fillStyle='rgba(220,241,255,.72)';ctx.font='700 9px system-ui';ctx.fillText('正在定位信号源……',W/2,H*.29+40)}else if(story.phase==='inspect'){ctx.fillStyle='rgba(3,12,27,.88)';ctx.fillRect(W/2-105,y+112,210,34);ctx.strokeStyle='rgba(85,224,255,.48)';ctx.strokeRect(W/2-105,y+112,210,34);ctx.fillStyle='#eafcff';ctx.font='900 15px system-ui';ctx.fillText('发现战机遗骸',W/2,y+135)}else if(['return','scan','reveal','briefing'].includes(story.phase)){ctx.strokeStyle=`rgba(92,235,255,${.42+pulse*.38})`;ctx.lineWidth=2;ctx.beginPath();ctx.arc(x,y,82+pulse*5,0,Math.PI*2);ctx.stroke();if(story.phase==='scan'){const progress=Math.max(0,Math.min(1,safeNumber(story.scanProgress,0))),scanY=y-82+164*progress;ctx.globalCompositeOperation='lighter';ctx.shadowBlur=18;ctx.shadowColor='#63edff';ctx.strokeStyle='rgba(170,251,255,.95)';ctx.lineWidth=2.5;ctx.beginPath();ctx.moveTo(x-92,scanY);ctx.lineTo(x+92,scanY);ctx.stroke();ctx.fillStyle='rgba(76,224,255,.06)';ctx.fillRect(x-82,y-82,164,164*progress)}}ctx.restore();
}
