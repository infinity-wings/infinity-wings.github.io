...
const ENEMY_ARCHIVE_DATA={
 ...
 suicide:{name:'自爆战机',baseHp:68,attack:'侧翼切入、有限追踪后直线冲锋',damage:'自爆 24 / 接触 28',xp:11,desc:'追踪1.35秒后锁定方向冲锋；固定圆形警戒环会随危险程度加快闪烁。'},
 ...
 jammer:{name:'干扰机',baseHp:62,attack:'双机或三机编队减速电网',damage:'电网 4 / 接触 16',xp:17,desc:'高速编队下压，机身之间形成减速带；击毁其中一机即可破坏对应连接。'},
 ...
 'boss-1':{name:'危险 I · 重甲先锋［前哨］',baseHp:'1953+',attack:'裂变重炮、缺口弹环、扇形弹幕',damage:'13–16',xp:480,desc:'重炮会在战场中段裂变，观察闪烁并穿过弹环缺口。'},
 'boss-2':{name:'危险 II · 弹幕母机［前哨］',baseHp:'2695+',attack:'双侧追踪弹、缺口弹环',damage:'13–17',xp:660,desc:'多炮组协同压制；追踪弹持续修正一段时间后会失去制导。'},
 'boss-3':{name:'危险 III · 裂隙猎手',baseHp:'5544+',attack:'引导扇形、瞄准弹、裂隙激光',damage:'17–28',xp:840,desc:'扇形火力迫使驾驶员移动，随后以预警激光封锁预计位置。'},
 'boss-4':{name:'危险 IV · 赤钢战争堡垒',baseHp:'6741+',attack:'赤钢封锁线、强化环形、扫射激光',damage:'17–28',xp:1020,desc:'固定封锁战场上沿，炮火墙始终保留一条可识别的安全通道。'},
 'boss-5':{name:'危险 V · 虚空航母',baseHp:'7938+',attack:'舰载机投放、虚空鱼雷、护航编队',damage:'17–20',xp:1460,desc:'以护航机和双侧追踪鱼雷制造多目标压力，本体弹幕密度相对克制。'},
 'boss-6':{name:'危险 Ω · 终焉核心',baseHp:'14159+',attack:'双层星环、移动弹墙、终焉激光',damage:'18–28',xp:'1640+',desc:'以连续安全通道检验走位，并在最终组合中叠加有预警的扫射激光。'}
};
...
function defaultSaveSlot(slots){
 const occupied=slots.map((save,index)=>({save,slot:index+1})).filter(item=>item.save).sort((a,b)=>new Date(b.save.updatedAt||0)-new Date(a.save.updatedAt||0));
 return occupied[0]?.slot||(slots.findIndex(save=>!save)+1)||1;
}
function renderSaveSlots(){
 const list=$('#saveSlotList');if(!list)return;const slots=IWSave.listSlots();
 if(selectedSaveSlot<1||selectedSaveSlot>slots.length)selectedSaveSlot=defaultSaveSlot(slots);
 list.innerHTML=slots.map((save,index)=>{const slot=index+1,summary=saveSlotSummary(save),active=selectedSaveSlot===slot;if(!summary)return `<article class="save-slot empty ${active?'active':''}" data-save-slot="${slot}" tabindex="0" role="button" aria-label="选择空存档 ${slot}"><span class="save-slot-number">SLOT ${slot}</span><span class="save-slot-main"><b>空存档</b><span>选择后可开始新的行动</span></span></article>`;return `<article class="save-slot ${active?'active':''}" data-save-slot="${slot}" tabindex="0" role="button" aria-label="选择存档 ${slot}"><span class="save-slot-number">SLOT ${slot}</span><span class="save-slot-main"><b>${summary.title}</b><span>${summary.detail}</span><small>${summary.time} · ${new Date(save.updatedAt).toLocaleString('zh-CN')}</small></span><button class="save-slot-delete" type="button" data-delete-slot="${slot}">删除</button></article>`}).join('');
 list.querySelectorAll('[data-save-slot]').forEach(card=>{const select=()=>{selectedSaveSlot=Number(card.dataset.saveSlot);renderSaveSlots()};card.onclick=e=>{if(e.target.closest('[data-delete-slot]'))return;select()};card.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();select()}}});
 list.querySelectorAll('[data-delete-slot]').forEach(button=>button.onclick=e=>{e.stopPropagation();const slot=Number(button.dataset.deleteSlot);if(!confirm(`确定删除存档 ${slot} 吗？此操作无法撤销。`))return;IWSave.deleteSlot(slot);if(selectedSaveSlot===slot)selectedSaveSlot=0;renderSaveSlots();refreshChapterMenu();toast(`存档 ${slot} 已删除`,'important')});
 const action=$('#saveSlotActionButton'),save=slots[selectedSaveSlot-1],summary=saveSlotSummary(save);action.disabled=!selectedSaveSlot;action.querySelector('span').textContent=save?'继续游戏':'开始游戏';action.querySelector('small').textContent=save?`载入 SLOT ${selectedSaveSlot} · ${summary.title}`:`在 SLOT ${selectedSaveSlot} 建立新的作战档案`;
}
