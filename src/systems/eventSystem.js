...
class BattleEventSystem{
 ...
 reset(){this.deck=[];this.active=null;this.nextAt=135+Math.random()*35;this.warning=null;this.enabled=true;this.spawnCd=0;this.fx=[];this.huntTarget=null;this.supportUnits=[];this.lastEventAt=-999;this.buildDeck();this.hideHud()}
 ...
 canStart(def=null){return running&&!paused&&!dying&&state==='game'&&!enemies.some(e=>e.boss)&&!build?.bossPendingNumber&&!awakeningSystem.pending&&!this.active&&!this.warning}
 update(dt){if(!this.enabled||!running||dying)return;this.updateFx(dt);const bossActive=enemies.some(e=>e.boss);if(bossActive&&(this.warning||this.active)){this.cancelForBoss();return}if(this.warning){this.warning.time-=dt;this.showHud(this.warning.def,this.warning.time,true);if(this.warning.time<=0){const id=this.warning.def.id;this.warning=null;this.start(id)}return}if(!this.active){const nextDef=this.deck.length?BATTLE_EVENT_BY_ID[this.deck[0]]:null;if(nextDef&&elapsed>=this.nextAt&&this.canStart(nextDef)){this.deck.shift();this.warning={def:nextDef,time:5};this.showHud(nextDef,5,true)}return}this.active.time-=dt;this.showHud(this.active.def,this.active.time,false);this.updateActive(dt);if(this.active&&this.active.time<=0)this.end(false)}
 ...
 cancelForBoss(){
  ...
  this.fx=[];this.supportUnits=[];this.active=null;this.huntTarget=null;this.hideHud();this.nextAt=elapsed+28;
 ...
 start(id,forced=false){const def=BATTLE_EVENT_BY_ID[id];if(!def||enemies.some(e=>e.boss))return false;if(this.active)this.end(true);this.active={def,time:def.duration,forced,success:false};this.spawnCd=0;this.lastEventAt=elapsed;toast(`${def.label} · ${def.name}`);if(id==='hunt_reversal')this.spawnHuntTarget();if(id==='fire_support')this.startFireSupport();return true}
 end(cancelled=false){if(!this.active)return;const id=this.active.def.id;if(id==='hunt_reversal'&&this.huntTarget&&this.huntTarget.hp>0&&!cancelled){this.huntTarget.enraged=true;this.huntTarget.bountyPhase=2;this.huntTarget.shoot=Math.min(this.huntTarget.shoot,520);this.huntTarget.hp=Math.min(this.huntTarget.max,this.huntTarget.hp+this.huntTarget.max*.28);toast('赏金猎杀失败 · 目标火力强化')}if(id==='meteor_rain')this.fx=this.fx.filter(f=>!['meteor','meteorWarning'].includes(f.type));if(id==='fire_support')this.supportUnits=[];this.active=null;this.huntTarget=null;this.hideHud();this.nextAt=elapsed+95+Math.random()*45}
 decorateSpawnedEnemy(enemy){return enemy}
 spawnMirrorPartner(enemy){
  if(this.active?.def.id!=='mirror_invasion'||!enemy||enemy.boss)return null;
