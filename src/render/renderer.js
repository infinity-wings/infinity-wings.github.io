...
function drawWreckStory(){
 ...
 ctx.save();ctx.translate(x,y+(story.float||0));ctx.rotate(story.rotation||0);ctx.globalAlpha=Math.max(0,Math.min(1,finite(story.alpha,1)));ctx.shadowBlur=story.phase==='return'?24:19;ctx.shadowColor=story.phase==='return'?'#5cecff':'#6ea8ff';ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';if(asset?.complete&&asset.naturalWidth){const size=story.phase==='signal'?154:176;ctx.drawImage(asset,-size/2,-size/2,size,size)}else{ctx.fillStyle='rgba(18,92,138,.72)';ctx.strokeStyle='#c8fbff';ctx.shadowBlur=26;ctx.shadowColor='#5cecff';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(0,-64);ctx.lineTo(64,36);ctx.lineTo(16,26);ctx.lineTo(0,58);ctx.lineTo(-16,26);ctx.lineTo(-64,36);ctx.closePath();ctx.fill();ctx.stroke()}ctx.restore();
