...
function drawEnemyShip(e,targetCtx=null){
 ...
 if(e.type==='suicide'){const distance=Math.hypot(player.x-e.x,player.y-e.y),proximity=Math.max(0,Math.min(1,1-(distance-55)/365)),frequency=3+proximity*18,flash=.18+.82*Math.pow(Math.max(0,Math.sin(e.age*frequency)),3);enemyModelCtx.save();enemyModelCtx.globalCompositeOperation='lighter';enemyModelCtx.globalAlpha=.22+.7*flash;enemyModelCtx.shadowBlur=10+proximity*14;enemyModelCtx.shadowColor=proximity>.65?'#ff2b32':'#ff9b35';enemyModelCtx.strokeStyle=proximity>.65?'#ff5260':'#ffc05c';enemyModelCtx.lineWidth=1.6+proximity*1.2;enemyModelCtx.beginPath();enemyModelCtx.arc(0,0,(e.r||14)+9+flash*2.5,0,Math.PI*2);enemyModelCtx.stroke();enemyModelCtx.restore()}
