...
function iwDefaultSave(){return{version:IW_SAVE_VERSION,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),profile:{pilotId:1,totalRuns:0},story:{currentChapter:1,chapterOneCompleted:false,chapterTwoUnlocked:false,riftUnlocked:false,wrecksFound:0,wrecks:[false,false,false,false],introSeen:false},progression:{highestThreat:0,cores:{},enemies:[],defeated:[]},runCheckpoint:null,records:[]}}
function iwNormalizeSave(raw){const base=iwDefaultSave(),save=raw&&typeof raw==='object'?raw:{};return{...base,...save,version:IW_SAVE_VERSION,profile:{...base.profile,...save.profile},story:{...base.story,...save.story,wrecks:Array.isArray(save.story?.wrecks)?save.story.wrecks.slice(0,4).map(Boolean):base.story.wrecks},progression:{...base.progression,...save.progression,cores:save.progression?.cores&&typeof save.progression.cores==='object'?save.progression.cores:{},enemies:Array.isArray(save.progression?.enemies)?save.progression.enemies:[],defeated:Array.isArray(save.progression?.defeated)?save.progression.defeated:[]},records:Array.isArray(save.records)?save.records.slice(0,20):[]}}
const saveSystem={
