...
class BattleEventSystem{
 ...
 reset(){this.deck=[];this.active=null;this.nextAt=135+Math.random()*35;this.warning=null;this.enabled=true;this.spawnCd=0;this.fx=[];this.gravity=null;this.huntTarget=null;this.supportUnits=[];this.mirrorPairs=[];this.lastEventAt=-999;this.buildDeck();this.hideHud()}
 ...
 cancelForBoss(){
  ...
  if(id==='mirror_invasion')for(let i=enemies.length-1;i>=0;i--)if(enemies[i].eventMirror||enemies[i].mirrorPairId)enemies.splice(i,1);
  if(id==='hunt_reversal'&&this.huntTarget){const i=enemies.indexOf(this.huntTarget);if(i>=0)enemies.splice(i,1)}
  this.fx=[];this.gravity=null;this.supportUnits=[];this.mirrorPairs=[];this.active=null;this.huntTarget=null;this.hideHud();this.nextAt=elapsed+28;
 ...
 if(id==='mirror_invasion'&&this.spawnCd<=0){this.spawnCd=2.8;const types=['scout','striker','barrage','sniper','jammer'];const type=types[Math.floor(Math.random()*types.length)];const offset=58+Math.random()*Math.max(30,W*.28);const y=70+Math.random()*Math.max(50,H*.22);const pairId='mirror-'+Math.round(elapsed*1000)+'-'+Math.floor(Math.random()*9999);const left=makeEnemy(type),right=makeEnemy(type);const hp=Math.max(18,Math.round(Math.max(left.max||left.hp,right.max||right.hp)*.7));Object.assign(left,{x:Math.max(30,W/2-offset),y,hp,max:hp,eventMirror:true,mirrorPairId:pairId,mirrorSide:-1,rewardless:true});Object.assign(right,{x:Math.min(W-30,W/2+offset),y,hp,max:hp,eventMirror:true,mirrorPairId:pairId,mirrorSide:1,rewardless:true});left.dir=1;right.dir=-1;enemies.push(left,right);this.mirrorPairs.push(pairId)}
 if(id==='emp_storm'&&this.spawnCd<=0){this.spawnCd=1.55+Math.random()*1.25;const x=45+Math.random()*(W-90),warningTime=1.18;this.fx.push({type:'lightningWarning',x,life:warningTime,max:warningTime,seed:Math.random()*99});setTimeout(()=>{if(!this.active||this.active.def.id!=='emp_storm'||enemies.some(e=>e.boss))return;this.fx.push({type:'lightningStrike',x,life:.38,max:.38});for(const e of [...enemies])if(Math.abs(e.x-x)<42&&damageEnemy(e,70)){const idx=enemies.indexOf(e);if(idx>=0)enemies.splice(idx,1)}if(Math.abs(player.x-x)<38)damagePlayer(13)},warningTime*1000)}
