...
function laserFire(level){
 ...
 for(const emitter of laserEmitterPositions(level)){
  bullets.push({laser:true,x:emitter.x,y:0,w:stats.width*(emitter.source==='clone'?.58:1),h:emitter.y-20,life:stats.duration,maxLife:stats.duration,d:stats.dps*(emitter.source==='clone'?.72:1),level,source:emitter.source,emitterOffset:emitter.offset,emitterIndex:emitter.emitterIndex,visualNodes:Array(9).fill(emitter.x)});
 }
...
function update(dt){
 ...
 for(let i=missiles.length-1;i>=0;i--){
  ...
  if(m.trail<=0){m.trail=particles.length>220?.07:particles.length>150?.045:.03;if(particles.length<280)particles.push({x:m.x,y:m.y,vx:-m.vx*.13+(Math.random()-.5)*22,vy:-m.vy*.13+(Math.random()-.5)*22,life:.22+Math.random()*.18,max:.4,r:2+Math.random()*2.5+(m.level||1)*.35,type:'missileTrail',missileLevel:m.level||1})}
